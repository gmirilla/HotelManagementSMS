<div class="mx-auto max-w-3xl">
    ...
    <div class="mb-6 flex items-center gap-2 text-xs font-medium text-slate-500">
        <span class="{{ $step >= 1 ? 'text-indigo-600' : '' }}">1. Dates</span>
        <span>&rarr;</span>
        <span class="{{ $step >= 2 ? 'text-indigo-600' : '' }}">2. Room</span>
        <span>&rarr;</span>
        <span class="{{ $step >= 3 ? 'text-indigo-600' : '' }}">3. Guest &amp; confirm</span>
    </div>
    ...
    @if ($step === 1)
        <div class="rounded-lg border border-slate-200 bg-white p-6">
            ...
            <button wire:click="searchAvailability" class="mt-6 rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">
                Search availability
            </button>
        </div>
    @endif
            ...
            @forelse ($this->availableRoomTypes as $row)
                <div class="flex items-center justify-between rounded-lg border border-slate-200 bg-white p-4">
                    ...
                    <div class="text-right">
                        ...
                        <button wire:click="selectRoomType({{ $row['roomType']->id }})" class="mt-1 text-sm font-medium text-indigo-600 hover:text-indigo-500">
                            Select &rarr;
                        </button>
                    </div>
                </div>
        ...
        <div class="rounded-lg border border-slate-200 bg-white p-6">
            ...
            @if (! $creatingNewGuest)
                <input type="search" wire:model.live.debounce.300ms="guestSearch" placeholder="Search existing guests&hellip;"
                    class="mb-3 block w-full rounded-md border-slate-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                ...
                <div class="mb-4 max-h-48 space-y-1 overflow-y-auto">
                    @foreach ($this->guestResults as $guest)
                        <button type="button" wire:click="selectGuest({{ $guest->id }})"
                            @class(['block w-full rounded-md px-3 py-2 text-left text-sm hover:bg-slate-50', 'bg-indigo-50' => $selectedGuestId === $guest->id])>
                            {{ $guest->fullName() }} <span class="text-slate-400">{{ $guest->email }}</span>
                        </button>
                    @endforeach
                </div>

                <button type="button" wire:click="$set('creatingNewGuest', true)" class="text-sm font-medium text-indigo-600 hover:text-indigo-500">
                    + New guest instead
                </button>
            @else
                <div class="grid grid-cols-2 gap-4">
            ...
            <div class="mt-6 flex gap-3">
                <button wire:click="confirm" class="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">
                    Confirm reservation
                </button>
