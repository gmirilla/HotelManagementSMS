<div>
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        @forelse ($this->roomTypes as $roomType)
            <div class="rounded-lg border border-slate-200 bg-white p-5" wire:key="room-type-{{ $roomType->id }}">
                ...
                <div class="mt-4 flex gap-3 text-sm">
                    @can('update', $roomType)
                        <button wire:click="edit({{ $roomType->id }})" class="font-medium text-indigo-600 hover:text-indigo-500">Edit</button>
                    @endcan
                    @can('delete', $roomType)
