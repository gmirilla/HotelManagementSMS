<div class="mx-auto max-w-3xl">
    <h1 class="mb-6 text-xl font-semibold text-slate-900">Walk-in check-in</h1>

    <div class="mb-6 flex items-center gap-2 text-xs font-medium text-slate-500">
        <span class="{{ $step >= 1 ? 'text-brand-600' : '' }}">1. Stay</span>
        <span>&rarr;</span>
        <span class="{{ $step >= 2 ? 'text-brand-600' : '' }}">2. Room type</span>
        <span>&rarr;</span>
        <span class="{{ $step >= 3 ? 'text-brand-600' : '' }}">3. Room</span>
        <span>&rarr;</span>
        <span class="{{ $step >= 4 ? 'text-brand-600' : '' }}">4. Guest &amp; check-in</span>
    </div>

    @if ($submitError)
        <div class="mb-4 rounded-md bg-red-50 px-4 py-3 text-sm text-red-800">{{ $submitError }}</div>
    @endif
    ...
    @if ($step === 1)
        <div class="rounded-xl border border-slate-200/70 bg-white shadow-sm shadow-slate-900/5 p-6">
            ...
            <button wire:click="proceedToRoomType" class="mt-6 rounded-md bg-brand-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-brand-500">
                Find rooms
            </button>
        </div>
    @endif
            ...
            <div class="flex flex-wrap gap-2">
                @forelse ($this->availableRooms as $room)
                    <button type="button" wire:click="selectRoom({{ $room->id }})"
                        class="rounded-md border border-slate-200 px-3 py-1.5 text-sm text-slate-600 transition hover:border-brand-300 hover:bg-brand-50/60 hover:text-brand-700">
                        {{ $room->room_number }}
                    </button>
                @empty
                    <p class="text-sm text-slate-500">No vacant rooms of this type at this branch right now.</p>
                @endforelse
            </div>

            <button wire:click="$set('step', 2)" class="text-sm text-slate-500 hover:text-slate-700">&larr; Change room type</button>
        </div>
    @endif
    ...
    @if ($step === 4)
        <div class="rounded-xl border border-slate-200/70 bg-white shadow-sm shadow-slate-900/5 p-6">
            ...
            @if (! $creatingNewGuest)
                ...
                @if ($this->selectedGuest?->isBlacklisted())
                    <div class="mb-4 rounded-md bg-amber-50 px-4 py-3 text-sm text-amber-800">
                        This guest is flagged as blacklisted.
                        @if ($this->selectedGuest->blacklist_reason)
                            {{ $this->selectedGuest->blacklist_reason }}
                        @endif
                        Check-in is still permitted — review before proceeding.
                    </div>
                @endif
            ...
            @else
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <x-input-error :messages="$errors->get('newGuestEmail')" />
                    </div>
                    <div>
                        <x-input-label for="newGuestPhone" value="Phone" />
                        <x-text-input id="newGuestPhone" type="text" wire:model="newGuestPhone" />
                    </div>
                    <div class="col-span-2">
                        <x-input-label for="newGuestNationality" value="Nationality" />
                        <x-text-input id="newGuestNationality" type="text" wire:model="newGuestNationality" />
                    </div>
                </div>

                <div class="mt-4 border-t border-slate-100 pt-4">
                    <h3 class="mb-3 text-sm font-medium text-slate-800">ID document (optional)</h3>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <x-input-label for="documentType" value="Document type" />
                            <select id="documentType" wire:model="documentType" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm sm:text-sm">
                            ...
                            <x-input-error :messages="$errors->get('documentType')" />
                        </div>
                        <div>
                            <x-input-label for="documentNumber" value="Document number" />
                            <x-text-input id="documentNumber" type="text" wire:model="documentNumber" />
                            <x-input-error :messages="$errors->get('documentNumber')" />
                        </div>
                        <div>
                            <x-input-label for="documentIssuingCountry" value="Issuing country" />
                            <x-text-input id="documentIssuingCountry" type="text" wire:model="documentIssuingCountry" />
                        </div>
                        <div>
                            <x-input-label for="documentExpiresOn" value="Expires on" />
                            <x-text-input id="documentExpiresOn" type="date" wire:model="documentExpiresOn" />
                        </div>
                    </div>
                </div>

                <button type="button" wire:click="$set('creatingNewGuest', false)" class="mt-4 text-sm text-slate-500 hover:text-slate-700">
            ...
            <div class="mt-6 flex gap-3">
                <button wire:click="confirmCheckIn" class="rounded-md bg-brand-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-brand-500">
                    Check in now
                </button>
                <button wire:click="$set('step', 3)" class="text-sm text-slate-500 hover:text-slate-700">&larr; Back</button>
            </div>
        </div>
    @endif
</div>
