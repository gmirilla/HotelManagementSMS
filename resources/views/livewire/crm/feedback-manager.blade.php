<div>
    ...
    <div class="space-y-3">
        @forelse ($this->feedback as $item)
            <div wire:key="feedback-{{ $item->id }}" class="rounded-xl border border-slate-200/70 bg-white shadow-sm shadow-slate-900/5 p-4">
                    ...
                    @if (! in_array($item->status->value, ['resolved', 'closed']))
                        <div class="mt-3 flex gap-3 border-t border-slate-100 pt-3">
                            @unless ($item->assignedTo)
                                <button wire:click="assignToMe({{ $item->id }})" class="text-sm font-medium text-brand-600 hover:text-brand-500">Assign to me</button>
                            @endunless
