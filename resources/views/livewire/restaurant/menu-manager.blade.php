<div>
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        @can('restaurant.manage')
            <button wire:click="$set('showOutletForm', true)" class="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">
                New outlet
            </button>
        @endcan
    </div>
    ...
    @if ($showOutletForm)
        <form wire:submit="saveOutlet" class="mb-6 flex items-end gap-3 rounded-lg border border-slate-200 bg-white p-4">
            <div>
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <div class="rounded-lg border border-slate-200 bg-white p-5">
                <div class="mb-3 flex items-center justify-between">
                    ...
                    @can('restaurant.manage')
                        <button wire:click="$set('showTableForm', true)" class="text-sm font-medium text-indigo-600 hover:text-indigo-500">+ Add</button>
                    @endcan
                </div>
                ...
                <div class="flex flex-wrap gap-2">
                    @foreach ($this->tables as $table)
                        <span @class([
                            ...
                            'bg-indigo-50 text-indigo-700' => $table->status->value === 'occupied',
            ...
            <div class="lg:col-span-2 rounded-lg border border-slate-200 bg-white p-5">
                <div class="mb-3 flex items-center justify-between">
                    ...
                    @can('restaurant.manage')
                        <button wire:click="addCategory" class="text-sm font-medium text-indigo-600 hover:text-indigo-500">+ Add category</button>
                    @endcan
                </div>
                ...
                @foreach ($this->categories as $category)
                    <div class="mb-4">
                        <div class="mb-2 flex items-center justify-between">
                            ...
                            @can('restaurant.manage')
                                <button wire:click="createItem({{ $category->id }})" class="text-xs font-medium text-indigo-600 hover:text-indigo-500">+ Item</button>
                            @endcan
                        </div>
                            ...
                            @foreach ($category->items as $item)
                                <div class="flex items-center justify-between py-1.5">
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        @can('restaurant.manage')
                                            <button wire:click="toggleAvailability({{ $item->id }})" class="text-xs text-indigo-600 hover:text-indigo-500">
